...
ROLL_PID = PID(Kp=0.1, Ki=0.0, Kd=0.0, setpoint=DESIRED[0], output_limits=(-DEFAULTS.Y_PID_LIMIT, DEFAULTS.Y_PID_LIMIT))
PITCH_PID = PID(Kp=0.1, Ki=0.0, Kd=0.0, setpoint=DESIRED[0], output_limits=(-DEFAULTS.Y_PID_LIMIT, DEFAULTS.Y_PID_LIMIT))
#THE IDEA IS WE GET CLOSE TO STABLE WITH THE MODEL AND ALL IMU STABILIZATION IS LOCAL IN X Y AND Z AXIS
#Y PROVIDES ORIENTATION BASED STABILITY (FLAT TRIM)
#X AND Z PREVENT PLATFORM FROM FALLING OVER
...
def callback(data):
    ...
    round(1.4756,2) we will use the round function to not have the motors jitter so much
    '''
    CMD_FLY = bound(OUTPUT_LIMIT_LOWER, OUTPUT_LIMIT_UPPER, round(CURRENT_SETPOINT[1]+ROLL_OUTPUT+PITCH_OUTPUT, 2))
    CMD_FRY = bound(OUTPUT_LIMIT_LOWER, OUTPUT_LIMIT_UPPER, round(CURRENT_SETPOINT[4]-ROLL_OUTPUT+PITCH_OUTPUT, 2))
    CMD_BLY = bound(OUTPUT_LIMIT_LOWER, OUTPUT_LIMIT_UPPER, round(CURRENT_SETPOINT[7]+ROLL_OUTPUT-PITCH_OUTPUT, 2))
    CMD_BRY = bound(OUTPUT_LIMIT_LOWER, OUTPUT_LIMIT_UPPER, round(CURRENT_SETPOINT[10]-ROLL_OUTPUT-PITCH_OUTPUT, 2))
